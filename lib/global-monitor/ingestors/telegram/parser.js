const sanitize = (value = '') => value.replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
...
export function parseTelegramPublicChannelHtml(html, channel) {
  const posts = [];
  const cards = html.match(/<div class="[^"]*tgme_widget_message_wrap[^"]*"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g) || [];
  ...
  cards.forEach((fragment) => {
    try {
      ...
      const title = bodyText ? bodyText.slice(0, 160) : null;
